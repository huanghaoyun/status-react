...
;; NOTE protection from double sending transaction
(def tx-timeout (atom nil))

(defn set-tx-timeout []
  (reset! tx-timeout (js/setTimeout #(reset! tx-timeout nil) 1000)))

(defn send-transaction [rpcParams password callback]
  (when (and status (nil? @tx-timeout))
    (set-tx-timeout)
